...
/*   Updated: 2025/03/22 20:46:25 by bchiki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     check_walls(t_game *game)
{
    ...
    while (i < game->height)
    {
        if (game->map[i * (game->width + 1)] != '1' || game->map[i * (game->width + 1) + game->width - 1] != '1')
            return (0);
        i++;
    }
    return (1);
...
int     check_accessibility(t_game *game)
{
    ...
    ft_printf("Starting at (%d, %d), collectibles: %d\n", game->player_x, game->player_y, game->collectibles);
    state = flood_fill(map_copy, game, state);
    if (state.collectibles > 0 && state.exit == 0)
        ft_putstr_fd("Error: Unreachable collectibles and exit\n", 2);
    else if (state.collectibles > 0)
        ft_putstr_fd("Error: Unreachable collectibles\n", 2);
    else if (state.exit == 0)
        ft_putstr_fd("Error: Unreachable exit\n", 2);
    else
        return (1);
    free(map_copy);
    return (0);
